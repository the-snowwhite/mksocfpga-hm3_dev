...
void usage(void)
{
	printf("mksocfpgamemio: Utility to read or write hm2socfpga memory locatons by commandline arguments\n Note: the mksocfpga uio0 driver needs to be active\n ");
...
//    printf("    mem pointer created OK\n");
	...
	if(argv[2][0] == '0' && argv[2][1] == 'x') {
		...
		printf("Decimal address value input found ");
	...
	switch (argv[1][1])
	{
		...
		case 'w':
			...
			if(argv[3][0] == '0' && argv[3][1] == 'x') {
				printf("Hex data value input found ");
				inval = (uint32_t) strtol(argv[3], NULL, 16);
			}
			else {
				printf("Decimal data value input found ");
